...
import { csNamespace } from './customsites-registry';

/**
 * Absolute URL (as a PageMetaImage, with known 1200x630 dims) to the
 * generated OG image fallback at app/<namespace>/og/route.tsx — used when
 * neither a page nor csSite has an explicit ogImage (a client-uploaded
 * ogImage always wins; callers must put this last in their `??` fallback
 * chain). The namespace comes from the registry so each site hits its own
 * palette-matched OG route. The `?cs=` query param makes the URL
 * self-resolving through proxy.ts regardless of Host header — needed for the
 * dev `?cs=`/`ll_cs` bypass, harmless (redundant) once the real domain is
 * attached in CUSTOM_HOSTS.
 */
export function csOgFallbackImage(baseUrl: string, siteKey: string): PageMetaImage {
  const namespace = csNamespace(siteKey) ?? 'cadr';
  return {
    url: `${baseUrl}/${namespace}/og?cs=${encodeURIComponent(siteKey)}`,
    width: 1200,
    height: 630,
  };
}
