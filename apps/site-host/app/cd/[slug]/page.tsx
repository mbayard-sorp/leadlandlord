 ...
 * app/cadr/[slug]/page.tsx with CueDuo's updates path vocabulary. */
export default async function CueDuoSlugPage({ params }: RouteParams) {
  ...
  if (pub) {
    ...
    const crumbs = await breadcrumbsJsonLd([
      { name: 'Home', path: '/' },
      { name: 'Updates', path: `/${insightsPath}` },
    ...
    return (
      <>
        ...
        <ArticleLayout pub={pub} related={related} phone={site.phone} indexPath={`/${insightsPath}`} indexLabel="Updates" />
      </>
    );
  }
