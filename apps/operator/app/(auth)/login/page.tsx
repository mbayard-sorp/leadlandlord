...
export default function LoginPage(props: SearchParams) {
  return (
    <div className="min-h-screen flex items-center justify-center px-4">
      <div className="w-full max-w-sm rounded-lg border border-slate-700 bg-slate-800/40 p-6">
        ...
        <Suspense fallback={<LoginForm next="/operator" />}>
          <LoginInner {...props} />
        </Suspense>
      </div>
    </div>
...
async function LoginInner({ searchParams }: SearchParams) {
  const { next, error } = await searchParams;
  return (
    <>
      {error && <p className="mb-4 text-sm text-red-400">Invalid password.</p>}
