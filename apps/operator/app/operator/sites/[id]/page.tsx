...
async function loadSiteDetail(id: string): Promise<SiteDetailData | null> {
  ...
  const [recentCalls, recentLeads, recentRuns] = await Promise.all([
  ...
  return { site: siteRow, recentCalls, recentLeads, recentRuns };
}
...
export default async function SiteDetailPage({ params }: Params) {
  ...
  if (!data) notFound();
  const { site, recentCalls, recentLeads, recentRuns } = data;
