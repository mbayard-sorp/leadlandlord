...
export default async function DraftReviewPage({ params }: PageProps) {
  ...
  const draftRejection = md.draftRejection as { reason: string; at: string } | undefined;
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 space-y-6 pb-24">
...
function Stat({
  ...
  return (
    <div>
      ...
      <div className={`${mono ? 'font-mono' : ''} ${warn ? 'text-amber-300' : 'text-slate-200'} text-xs`}>
        {value}
      </div>
    </div>
  );
...
function highlightAnchor(mdText: string, anchor: string | null): string {
  if (!anchor) return mdText;
  const idx = mdText.indexOf(anchor);
  if (idx < 0) return mdText;
  return `${mdText.slice(0, idx)}<mark>${anchor}</mark>${mdText.slice(idx + anchor.length)}`;
}
