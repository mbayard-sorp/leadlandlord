...
export interface AlertEvaluatorResult {
  ok: boolean;
  evaluated: number;
  fired: Array<{ rule: string; severity: string }>;
 ...
 *
 * Extracted from /api/cron/alert-evaluator so the consolidated /api/cron/tick
 * poll can share it. Channels are best-effort: a failed channel doesn't prevent
 * other channels from firing or stop the run. Failures get logged.
 */
export async function runAlertEvaluator(): Promise<AlertEvaluatorResult> {
  const db = getDb();
  ...
  return { ok: true, evaluated: rules.length, fired, skipped };
}
