...
| `molly-digest` | Superseded | Superseded by fleet-digest; disposition OFF. BL-025: cron trigger (vercel.json), seed row (seed-agent-schedules.ts), and scheduler registration (scheduler/index.ts) all removed — decommission complete. The agent itself remains registered and disposition-gated OFF, same precedent as closer-agent/billing-dunning/churn-recovery (BL-004). | 5 | 2026-07-20 |
