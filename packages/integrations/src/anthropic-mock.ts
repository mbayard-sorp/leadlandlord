...
/**
 * niche-validator annotation pass: one annotation per `id=<uuid>` line in the
 * prompt so the MOCK_AI end-to-end loop renders annotations on pending rows.
 */
function mockNicheAnnotations(prompt: string): unknown {
  const ids = prompt.match(/id=[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}/g) ?? [];
  return {
    annotations: ids.map((m) => ({
      niche_id: m.slice(3),
      seasonality: 'year-round demand (mock)',
      licensing_concern: null,
      caution: 'Mock annotation generated by MOCK_AI=true.',
    })),
  };
}
