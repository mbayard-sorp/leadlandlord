...
function mockFaqPages(prompt: string): unknown {
  const nicheMatch = /niche[:\s]+([^\n]+)/i.exec(prompt);
  const cityMatch = /city[:\s]+([^\n]+)/i.exec(prompt);
  const niche = nicheMatch?.[1]?.trim() ?? 'mock niche';
  const city = cityMatch?.[1]?.trim() ?? 'Mock City';
  const ns = (s: string) => s.toLowerCase().replace(/\s+/g, '-');
  const mk = (slug: string, title: string) => ({
    slug: `/faq/${slug}`,
    title,
    meta_description: `Mock answer to "${title}" — generated by MOCK_AI=true for backfill testing.`,
    mdx: `This is mock FAQ content answering "${title}" for ${niche} in ${city}. The MOCK_AI flag bypassed the real Anthropic API to keep your $0 spend during pipeline testing.\n\n## More detail\n\nLorem ipsum dolor sit amet.`,
  });
  return {
    faq_pages: [
      mk(`how-much-does-${ns(niche)}-cost-in-${ns(city)}`, `How much does ${niche.toLowerCase()} cost in ${city}?`),
      mk(`do-you-serve-${ns(city)}`, `Do you serve ${city}?`),
      mk(`how-long-does-${ns(niche)}-take-in-${ns(city)}`, `How long does ${niche.toLowerCase()} take in ${city}?`),
    ],
  };
}
