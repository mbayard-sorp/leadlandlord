...
export const csPublication = defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      type: 'number',
      description: 'Podcast episode number. Leave empty for anything that isn’t an episode.',
    }),
    defineField({
      name: 'durationMinutes',
      title: 'Duration (minutes)',
      type: 'number',
      description: 'Runtime in whole minutes. Rendered as "42 min".',
    }),
    defineField({
      name: 'listenLinks',
      title: 'Listen Links',
      ...
      of: [
        {
          ...
          fields: [
            defineField({
              ...
              title: 'Platform',
              type: 'string',
              options: { list: ['spotify', 'apple', 'youtube', 'other'] },
              validation: (r) => r.required(),
            }),
            defineField({ name: 'url', title: 'URL', type: 'url', validation: (r) => r.required() }),
            defineField({ name: 'label', title: 'Label', type: 'string', description: 'Only needed for "other".' }),
          ],
          preview: { select: { title: 'platform', subtitle: 'url' } },
        },
      ],
      validation: (r) => r.max(5),
      description: 'Per-platform listen links. Platform brand colors are not authorable; they live in CSS as fixed per-platform values.',
    }),
