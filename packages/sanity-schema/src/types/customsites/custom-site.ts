...
export const csSite = defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      group: 'seo',
      readOnly: true,
      description: 'Auto-generated 32-character lowercase hex key, served at /{key}.txt on this host, that makes IndexNow verification work on a custom host. Generated and patched by scripts/customsites-indexnow.ts on its first run for this site — do not hand-edit.',
      validation: (r) => r.regex(/^[a-f0-9]{32}$/, { name: 'lowercase-hex-32' }),
    }),
    defineField({
      name: 'titleTemplate',
      ...
      type: 'string',
      group: 'seo',
      description: 'Metadata title template, e.g. "%s | Michael J. Bayard Construction ADR". When empty, the renderer falls back to `%s | ${site.name}` — leave blank rather than guessing a brand string.',
    }),
    defineField({
      name: 'areaServed',
      title: 'Area Served',
      ...
      of: [{ type: 'string' }],
      group: 'seo',
      description: 'Cities, counties, or regions the practice serves, e.g. "Los Angeles County", "Southern California". Feeds the schema.org LegalService areaServed JSON-LD node. Distinct from Address (the office location) — a mediator/arbitrator practice typically serves a wider region than its office city.',
    }),
    defineField({
    ...
    defineField({
      ...
      rows: 3,
      group: 'content',
      description: 'Short compliance line shown above the footer legal row on every page. Plain text, no links — link out via Footer Navigation instead.',
    }),
